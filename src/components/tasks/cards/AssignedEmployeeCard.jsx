...
const AssignedEmployeeCard = ({ taskDetail, passSelectedEmployee }) => {
  return (
    <div className="w-full flex flex-col gap-1 justify-start items-start">
      ...
      {taskDetail?.assignTo?.map((employee, index) => (
        <span
          key={index}
          className="w-full  grid grid-cols-5 py-1 text-[13px] font-medium leading-[14.85px] text-white justify-start items-center"
        >
          ...
          <span className="w-full flex justify-start items-center ">
            {employee?.location ?? "---"} asldjsalk salkjdsalkjds saojdlksad
            saojdlksad salkjdlksa aslkjdlksad lksjdlksajd
          </span>
        </span>
      ))}
    </div>
