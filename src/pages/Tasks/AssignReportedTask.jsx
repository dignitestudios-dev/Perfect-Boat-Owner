...
const AssignReportedTask = () => {
  ...
  const handleAssignTask = async(data) => {
    console.log("🚀 ~ handleAssignTask ~ data:", data)
    try{
  ...
  useEffect(() => {
    ...
    if (task) {
      ...
      setPassSelectedEmployee({name:task?.employee?.name, id:task?.employee?._id});
      setDueDate({ normal: moment(task?.task?.dueDate * 1000).format('YYYY-MM-DD'), unix: task?.task?.dueDate});
      setSelectedDay(task?.task?.reoccuringDays ? `${task?.task?.reoccuringDays} days` : "None");
  ...
  return (
    <div className="w-full h-auto min-h-screen overflow-y-auto bg-[#1A293D] text-white p-4 pb-20 flex flex-col justify-start items-start">
      ...
      <div className="w-full grid grid-cols-1 gap-12">
        <div className="w-full h-auto flex flex-col gap-1 justify-start items-start">
          ...
          <div className="flex items-center justify-between">
            ...
            <p
              onClick={(e) => { e.preventDefault(); setIsEmployeeModalOpen(true); }}
          ...
          <div className="w-full grid grid-cols-2 gap-12">
          <div className="group transition-all duration-500 w-full h-[52px] bg-[#1A293D] outline-none flex justify-between 
          items-center  px-3 focus:border-[1px] focus:border-[#55C9FA] rounded-xl  relative">
            <span className="text-gray-400">{passSelectedEmployee?.name || "Assign Employee"}</span>
          </div>
          </div>
          ...
          <DateModal isOpen={isCalendarOpen} setIsOpen={setIsCalendarOpen} setDueDate={setDueDate}/>
        </div>
      </div>
      <div className="w-full flex justify-end mt-10 items-center gap-4">
        ...
        >
          <div className="flex items-center"><span className="mr-1">Assign Reported Task</span>{submitLoading &&(
